...
// array for saving data from PV GIS
float* PVData = allocateFloatArray(arraySize);
// array to save max power from sensor for every hour
...
// used to save results from one day
struct ergebnisTag ErgebnisWoche[7] = {};
bool comparingStarted = false;


void initLittleFS();
float get_Altersfaktor(float given_faktor, int alter);
...
// used to calculate the Altersfaktor for the solar panels
...
// checks if solar panels broke down and provided no power this week
bool check_breakdown(struct ergebnisTag ErgebnisWoche[7]){
...
// processes messaage read by sensor
void process_message(byte *buffer, size_t len, Sensor *sensor, State sensorState)
{
...
// compares all values from one day read by sensor with PV GIS data
struct ergebnisTag CompareOneDay(float Vergleich[], float Mess[], float altersfaktor, int index, int kWp) {
...
void setup() {
  ...
  // create Sensor Task and run forever
  xTaskCreate([](void*) {
