...
// function to connect to WiFi, send email with IP on success
bool connectToWiFi() {
...
// used to start credentials server where Wifi and Email credentisl can be provided
...
// web server task. Starts WebServer if wifi is connected, if no or wrong wifi credentials: credentials server is started
void webServerTask(void * parameter) {
...
// task taht runs Credentials Server
void credentialsTask(void * parameter){
  Serial.println("Credentials Task started");
...
// funciton used to start webserver. All web functions implemented here
void startWebServer(const char* htmlPage) {
  // Serve HTML form
  ...
  // can be used to automatically download from PV GIS. Not used because of crashes
...
// function to read hmtl files to string for webserver
String readHTMLFile(const char * filePath) {
  String htmlContent;
...
// get wifi credentials for display on webPage
String getSavedWiFiCredentials() {
...
// get email credentials for display on webPage
String getSavedEmailCredentials() {
...
// read PV parameters from file for usage in calcualtions etc
bool readParametersFromFile(const char* path) {
  // Open the JSON file
...
// Task to handle file download from Filesytsem to user
void handleFileDownload(AsyncWebServerRequest *request) {
